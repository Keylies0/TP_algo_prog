...
public class Node {
    ...
    /* ------------------ constructeur ------------------ */
    public Node(State state, Node node, HalvingGame.Action action) {
        this.state = state;
        this.parent = node;
    ...
    public List<Node> expand(HalvingGame problem) {
    ...
    private boolean isActionValid(HalvingGame.Action action) {
        return true; // ????
    }

    /** Génère un nœud enfant pour une action donnée. Si l'action n'est pas valide, retourne null */
    private Node buildChild(HalvingGame problem, HalvingGame.Action action) {
